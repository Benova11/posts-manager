...
export class PostsService {
  ...
  addPost(title: string, content: string, image: File) {
    ...
    this.http
      ...
      .subscribe(response => {
        const newPost: Post = {id: response.post.id, title, content, imagePath: response.post.imagePath};
        this.posts.push(newPost);
        this.newDataInsertedNavigator();
      });
  }
  ...
  updatePost(id: string, title: string, content: string, image: File | string) {
    ...
    this.http.put<Post>('http://localhost:3000/api/posts/' + id, postData)
      .subscribe((resData) => {
        ...
        const post: Post = {id, title, content, imagePath: resData.imagePath};
